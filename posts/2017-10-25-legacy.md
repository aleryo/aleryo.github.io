...
compilé[^1] vers du C++. Au dessus de ce langage socle, l'équipe a développé un
...
QQQ Expliquer pourquoi c'est inacceptable?
...
La forme de cette application legacy est à un plus d'un titre peu motivante.
L'équipe en place avait estimée qu'il était impossible de faire des tests
automatisés à l'interieur de ce code. La seule stratégie envisagée était de
faire des tests de non regression manuels externalisé vers des pays à bas couts.

Nous avons pris le parti d'aborder cette application avec détermination, sans
nous décourager, en restant focalisés sur la valeur métier produite et en
évitant des effets tunnels de refactoring. Nous n'avons pas chercher à utiliser
ou créer des frameworks compliqués. Nous nous sommes essentiellement appuyés
sur des fonctionnalités disponibles nativement dans les langages utilisés.

Et cela a été payant.

Il est vrai aussi que nous avons été aidé par la forme plus testable du backend.
Le front-end reste encore aujourd'hui essentiellement une zone sinistrée.

Ce début de harnais de tests pourrait servir de graine pour aller vers des
refactoring plus larges, et pourquoi pas, vers une migration technolique
attendue avec une certaine impatience par toute l'équipe.

Nous n'avons pas baissés les bras sur la partie front-end. Un refactoring progressif
est probablement trop risqué et couteux mais nous travaillons déjà sur une stratégie
qui permettrait de faire croitre un deuxieme front-end à coté de celui qui existe
et qui pourrait le remplacer dans un horizon qui pourrait etre plus proche que
ce que l'histoire de l'application pourrait nous laisser penser.
